...
for FILE in ${CFGS}; do
  case $FILE in
    ...
    *.xml)
        ...
        sed -i "/<effects>/ a\        <effect name=\"v4a_standard_re\" library=\"v4a_aidl\" uuid=\"90380da3-8536-4744-a6a3-5731970e640f\"\/>" $FILE
        ;;
  esac
done
...
if [ -d "/odm/etc/" ]; then
  ...
  mount -o bind /data/adb/modules/ViPER4Android-RE-AIDL/odm/etc/audio_effects.xml /odm/etc/audio_effects.xml
fi
